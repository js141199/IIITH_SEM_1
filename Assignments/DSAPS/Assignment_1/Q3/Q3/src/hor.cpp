...
void removeHorizontalSeam(int ***rgb,double **horizontalMinCost,int H,int W){
    ...
    for(int h = 0; h < H; h++){
        minCost = minCost > horizontalMinCost[h][w] ? horizontalMinCost[h][w] : 1e9;
...
void makeHorizontalMinCostMatrix(double **greyScale,double **horizontalMinCost,int H,int W){
    ...
    for(int w = 1; w < W; w++){
        for(int h = 0; h < H; h++){
            ...
            horizontalMinCost[h][w] = greyScale[h][w] + min(leftDiagonal, min(rightDiagonal,backSide));
...
void convertRGBToGreyScale(int ***rgb,double **greyScale,int x,int y,int H,int W){

    double rx,ry,gx,gy,bx,by,addx,addy;
...
// ------------ Vertical Seam carving starts ------------------------------------------------
    ...
    while(W != W_ ){
        ...
        for(int x = 0; x < H ; x++){
            ...
            for(int y = 0; y < W ; y++){
                convertRGBToGreyScale(rgb,greyScale,x,y,H,W);
            }
        }
...
//----- Horizontal Seam carving starts ----------------
    ...
    while(H != H_){
        ...
        // first step find grey scale of 
        double **greyScale;
        greyScale = new double*[H];

        for(int h = 0; h < H; h++){
            greyScale[h] = new double[W];
            for(int w = 0; w < W; w++){
                convertRGBToGreyScale(rgb,greyScale,h,w,H,W);
            }
        }
