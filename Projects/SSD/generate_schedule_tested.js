...
function node(id,duration,count) {
        ...
        if(slots[i].length > 0){
            ...
            if(breaks.length > 0){
                ...
                events.forEach(element => {
                    if(eventsDone != 0 && eventsDone % breakFactor == 0 && j < breaks.length){
                        // take a break
...
function createSchedule(events,breaks){
    ...
    // cout << " eventsCount:" << eventsCount << endl;
...
fetch ("./new_input.json").then (response => response.json()).then (response => parseJsonData (response));
    ...
    if(slots[0].length > 0){
        ...
        slots[0].forEach(element => {
            ...
            events.push({
                ...
                "eventDescription":  (element.third == 1 ? data.eventDescription : data.breakDescription),
                "eventVenue" : (element.third == 1 ? data.eventVenue : data.breakVenue),
                "eventStartTime": getTime(Math.floor(currentTime/60)) + ":" + getTime (currentTime%60),
                "eventEndTime" : getTime (Math.floor((currentTime+element.second)/60)) + ":" + getTime ((currentTime+element.second)%60),
